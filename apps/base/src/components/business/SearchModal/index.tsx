...
import AntdIcons from '../../common/AntdIcons';

interface ScarchProps {
  isShow: boolean;
  onEnter: (item: API.MenuType['nodeData']) => void;
}
const SearchModal: FC<ScarchProps> = ({ isShow = true, onEnter }) => {
  ...
  const [routeList, setRouteList] = useState<API.MenuType['nodeData'][]>([]);
  const [menuList, setMenuList] = useState<API.MenuType['nodeData'][]>([]);
          ...
          {menuList.map((item, i: number) => (
            ...
            >
              <div className={styles.icon}>
                <AntdIcons name={(item.icon || 'MenuFoldOutlined') as keyof typeof antIcons} />
              </div>
